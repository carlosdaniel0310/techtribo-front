document.addEventListener("DOMContentLoaded", () => {
  ...
  if (id) {
    // Página de detalhes
    fetch(`http://localhost:3000/desenvolvedores/${id}`)
      .then(res => res.json())
      .then(dev => {
    ...
    // Página index
    fetch("http://localhost:3000/desenvolvedores")
      .then(res => res.json())
      .then(devs => {
...
function toggleFavorito(dev) {
  ...
  if (jaExiste) {
    favoritos = favoritos.filter(f => f.id !== dev.id); // remover
  } else {
    favoritos.push(dev); // adicionar
  }
